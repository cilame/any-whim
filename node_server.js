function make_js(func){
return (func+'').replace(/^function [^\(]+\(\)\{|\}$/g, '').trim()
}
function js_axios(){
...
function js_init(){
function mk_form(params){ return Object.keys(params).map(function(v){ return v + '=' + encodeURI(params[v]) }).join('&') }
function send_input(){
  var formdict = { data: input.value }
  axios.post('/test', mk_form(formdict))
  .then(function(response){ updatetime(); output.value = response.data.output.data })
  .catch(function(error){ console.log(error) })
}
function updatetime(){ time.innerText = +new Date+'' }
updatetime()
}
...
<body>
<style>
textarea{ width:1200px; height:300px }
label{ margin-left:10px }
</style>
<div>功能说明：用于测试接口</div><hr>
<button onclick="send_input()">向服务器传入内容</button><label id="time"></label><hr>
<textarea id="input" size="20" placeholder="输入内容">${testdata}</textarea><br>
<textarea id="output" size="20" placeholder="输出内容"></textarea><br>

<script type="text/javascript">${make_js(js_axios)}</script>
<script type="text/javascript">${make_js(js_init)}</script>
</body>
</html>`

var INTERFACE_POST = 8000
var router = express.Router();
router.get('/test', function(req, res){
  res.json({message: 'pls use post.'}) // req.query
})
router.post('/test', function(req, res){
  res.json({message: 'ok.', output: vilame(req.body)})
})

router.get('/', function(req, res){ res.send(html) })
var app = express();
app.use(bodyParser.urlencoded({ extended: true }));
app.use(bodyParser.text());
app.use('/', router);
app.listen(INTERFACE_POST, function(){
  console.log('test server start @', INTERFACE_POST)
})