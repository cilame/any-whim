...
class CreateCate(object):
    ...
    def __init__(self, *args, **kw):
        ...
        **kw:
        ...
        self.__cates = list(map(tuple, np.eye(self.class_num).tolist()))
    ...
    def save_mapdict(self, name):
        ...
        会生成 cls.pickle 文件
        """
        pickle.dump(self.__cates2class, open(name,'wb'))
        ...
        for cls in self.classes:
            for picpath in self.__files[cls]:
                for pic in self.__create_sample(picpath):
                    ...
                    x += [self.__cv_read(pic).astype(self.__nptype)]
